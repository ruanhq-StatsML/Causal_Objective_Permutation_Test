...
recursively, applying the same two-stage selection at each level (a per-subset
permutation test plus a bootstrap stability threshold, gated), and never report
proportions:

\begin{enumerate}[leftmargin=2em]
\item \textbf{Detect} the shift globally (MMD / $R$-risk permutation test).
\item \textbf{Localize (level 1)} to a responsible attribute-group subset on the
current axis.
\item \textbf{Drill (level 2)} by re-running the same selection on the next
aggregation key (e.g.\ item, region), \emph{conditioned} on the attribute flagged
upstream --- again returning a subset, not a share.
...
\paragraph{Drill one level.} Re-running the same two-stage selection on the
\emph{item} axis, conditioned on the order-count signal, localizes the extra
orders to a specific \textbf{product vertical} (item category $C$): within region
$R$, the increase concentrates in category $C$ (e.g.\ ``fresh grocery''), not
uniformly across the catalogue. Output: subset $=\{$region $R$, user segment,
category $C\}$ with its own split rule and stability.
