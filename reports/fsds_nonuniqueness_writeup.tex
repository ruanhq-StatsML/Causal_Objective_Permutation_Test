...
concentrated in a single product vertical $c^\star$ for new merchants
(200 merchants, 6 verticals, 24k orders) and run the procedure. Level 1
(merchant axis) selects the \texttt{gmv} attribute group by LOGO ablation with
full stability; the conditional Level 2 (product vertical, new-vs-existing GMV
with merchant-block permutation) isolates exactly $c^\star$ while every other
vertical is non-significant (Table~\ref{tab:twolevel}). No proportions are
reported --- only the selected subsets, their ablation/gap magnitude, stability,
and permutation $p$.
...
\;\; & quantity & $-0.277$ & $0.85$ & --- \\
\midrule
\multicolumn{5}{l}{\emph{Level 2 --- product vertical} \;(conditional on gmv, new vs.\ existing)}\\
\;\; & \textbf{vertical $c^\star{=}2$} \;(selected) & $55.88$ & --- & $0.002$ \\
\;\; & vertical 3 & $0.77$ & --- & $0.158$ \\
\;\; & vertical 1 & $0.58$ & --- & $0.240$ \\
\;\; & vertical 0 & $0.29$ & --- & $0.621$ \\
\;\; & vertical 4 & $0.26$ & --- & $0.583$ \\
\;\; & vertical 5 & $0.03$ & --- & $0.932$ \\
\bottomrule
\end{tabular}
\caption{Two-level conditional subset localization (`fsds\_cross\_level.py`).
Level 1 selects the \texttt{gmv} attribute subset (LOGO ablation $+$ bootstrap
stability); the conditional Level 2 selects exactly the shifted product vertical
$c^\star$ (gap $=$ new$-$existing mean GMV within the vertical; merchant-block
permutation $p$). Recovers ground truth at both levels.}
\label{tab:twolevel}
\end{table}
