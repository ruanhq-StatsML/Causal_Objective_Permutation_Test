...
set with a simple two-level per-subset permutation selection and subsampling
stability selection. The decision layer estimates covariate and
...
\section{Two-level selection by permutation and stability}

We keep the selection deliberately simple: a two-level attribution whose purpose
is to produce structured insights, not a formal multiple-testing guarantee. We
avoid the Benjamini--Hochberg (BH) family (few, strongly correlated group tests
where its independence/PRDS assumptions are dubious and it is under-powered) and
also do \emph{not} impose family-wise (e.g.\ Westfall--Young) control; each
subset is judged on its own permutation $p$-value.

\paragraph{Stage 1 (groups).} Rank groups by Shapley-MMD; select
$\Gcal^\star=\{g: p_g<\alpha \text{ and } \hat\pi_g\ge\pi_{\mathrm{thr}}\}$,
where $p_g$ is the group's MMD permutation $p$-value (exact under exchangeability)
and $\hat\pi_g$ its subsampling selection frequency (Section~\ref{sec:stability}).
The stability threshold suppresses flukes without a global correction.

\paragraph{Stage 2 (features within a selected group).}
Within each $g\in\Gcal^\star$, compute a per-feature MMD permutation $p$-value
$p_j$ for $j\in g$ and select $\{j: p_j\le\alpha\}$. No multiplicity adjustment
is applied.

The same rule applies at every split of the multi-resolution tree
$\text{root}\to\text{attribute}\to\text{aggregation family}\to\text{feature}$,
and the tree is \emph{gated}---a node is entered only if its parent was
selected. Each node is re-tested on its own feature subset (marginal at the
root, increasingly conditional deeper down): \emph{subset post-hoc localization}
conditional on a significant global test.

\begin{remark}[Cost of dropping multiplicity control]
Without a family-wise correction the per-comparison Type-I error is not pooled,
so a few weak non-driver groups/features may be selected. In practice they carry
low priority ($=\text{exposure}\times\text{impact}$) and stability, so the
structured insights rank them well below the true drivers; empirically the true
drivers are always recovered (recall $=1$) with only occasional low-ranked
extras.
\end{remark}
...
\item Stage 1: Shapley/LOGO ranking; group permutation $p_g$ and stability
      $\hat\pi_g$ $\to$ $\Gcal^\star=\{p_g<\alpha,\ \hat\pi_g\ge\pi_{\mathrm{thr}}\}$.
\item Stage 2: within each $g\in\Gcal^\star$, per-feature permutation $p_j$
      $\to$ selected features $\{p_j\le\alpha\}$ (no multiplicity adjustment).
