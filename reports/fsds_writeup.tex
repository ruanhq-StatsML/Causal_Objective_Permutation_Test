...
set with Benjamini--Bogomolov hierarchical false-discovery-rate (FDR) control
and subsampling stability selection. The decision layer estimates covariate and
...
\section{Two-stage selection with hierarchical FDR}

\paragraph{Stage 1 (groups).} Test each group $g$ (standalone MMD permutation
$p$-value $p_g$) and select $\Gcal^\star=\{g:\text{BH}(\{p_g\})\text{ rejects at }q_1\}$;
let $R_1=|\Gcal^\star|$, $m_1=|\Gcal|$.

\paragraph{Stage 2 (features within a selected group).} Within each
$g\in\Gcal^\star$, test its features and apply BH at the \emph{deflated} level
$q_2^{\mathrm{eff}}=q_2\,R_1/m_1$.

\begin{proposition}[Hierarchical FDR; Benjamini--Bogomolov]
If Stage 1 controls the FDR over groups at $q_1$ and each selected family is
tested at $q_2 R_1/m_1$, then the expected proportion of falsely selected
features over the tree is controlled at approximately $q_2$. Families not
selected in Stage 1 are never entered, which increases power and prevents
leakage.
\end{proposition}

The procedure generalizes to a multi-resolution tree
$\text{root}\to\text{attribute}\to\text{aggregation family}\to\text{feature}$
by deflating the level by $R/m$ at each split; each node is re-tested on its own
feature subset (marginal at the root, increasingly conditional deeper down),
i.e.\ \emph{subset post-hoc localization} conditional on a significant global
test.
...
\item Stage 1: group standalone-MMD $p$-values $\to$ BH at $q_1$ $\to$ $\Gcal^\star$;
      Shapley/LOGO ranking; bootstrap stability $\hat\pi_g$.
\item Stage 2: within each $g\in\Gcal^\star$, BH at $q_2 R_1/m_1$ $\to$ selected features.
...
\bibitem{yekutieli} D.\ Yekutieli. \emph{Hierarchical FDR}. JASA, 2008.
